...
A minimal **web client** for [Herdr](https://herdr.dev). It talks to a
...
Herdr must be installed and running before starting Herdr Studio. The installer
...
[latest release](https://github.com/powerfooI/herdr-studio/releases/latest), verify
...
For day-to-day use, we recommend installing Herdr Studio as a standalone web app
...
sockets or Windows named pipes; they never start Herdr. When the first profile is created, the
...
optional: leave them empty and Herdr Studio resolves the default Herdr sockets
...
`~/.ssh/config`; Herdr Studio uses the normal OpenSSH host-key and agent/Keychain
policies and does not store passwords, private keys, passphrases, or arbitrary
SSH options. SSH profiles and `--ssh-host` currently require Herdr Studio to run on
...
| `service install --force` | Replace an existing definition not generated by Herdr Studio |
...
A new service listens on `0.0.0.0:8787`, creates the persistent login token,
and prints tokenized localhost and LAN URLs during installation. The command
creates `~/.config/herdr-gui/herdr-gui.env` on Unix or
`%APPDATA%\herdr-gui\herdr-gui.env` on Windows and preserves it on reinstall
or uninstall. Edit that file for `HOST`, `PORT`, an optional fixed password,
and Herdr connection settings, then run `herdr-gui service restart`. Use
`herdr-gui service reload` after editing the generated platform definition;
it reloads the definition and restarts the process. On Windows, approve the
Task Scheduler or firewall prompt if one appears; allow Private networks only,
or set `HOST=127.0.0.1` before install for local-only access.

The random token is stored in `~/.config/herdr-gui/auth-token` on Unix and
`%APPDATA%\herdr-gui\auth-token` on Windows.
